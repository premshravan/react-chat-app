...
const Chat = () => {
  ...
  const [text, setText] = useState(""); // imogi value to the input field
  ...
  const handleDeleteMessage = async (message) => {
    try {
      ...
      // Update the messages in the "chats" collection
      await updateDoc(doc(db, "chats", chatId), { messages: updatedMessages });
  
      // Find the latest remaining message
      const latestMessage = updatedMessages.length
      ...
      userIDs.forEach(async (id) => {
        ...
        if (userChatsSnapshot.exists()) {
          ...
          if (chatIndex > -1) {
            ...
            userChatsData.chats[chatIndex].lastMessage = latestMessage;
            userChatsData.chats[chatIndex].updatedAt = Date.now();
